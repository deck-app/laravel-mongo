...
#########################
# wait_for_db() {
#     local -r db_host="${DB_HOST:-mongo}"
#     local -r db_port="${DB_PORT:-27017}"
#     local db_address
#     db_address=$(getent hosts "$db_host" | awk '{ print $1 }')
#     local counter=0
#     log "Connecting to mongo at $db_address"
...
#             exit 1
#         fi
#         log "Trying to connect to mongo at $db_address. Attempt $counter."
#         sleep 5
#     done
# }
